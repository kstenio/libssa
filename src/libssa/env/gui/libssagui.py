...
from numpy import std, ones, int16, zeros, arange, hstack, ndarray, linspace
from pandas import DataFrame, read_excel
from PySide6 import QtGui, QtWidgets
from pyqtgraph import TextItem, PlotWidget, BarGraphItem, mkPen, mkBrush, setConfigOption
from numpy.random import rand, random, randint, uniform
...
class LIBSsaGUI(QtWidgets.QMainWindow):
	...
	def __init__(self, uifile: str, logofile: str):
		...
		# If no error was found, loads all remaining widgets
			...
			self.menu_export_peaks_table = self.menu_export_peaks_isolated = self.menu_export_peaks_fitted = (
				self.menu_export_peaks_areas
			) = QtGui.QAction()
			...
			self.p4_areas = self.p4_heights = self.p4_wnorm = self.p4_pnorm = self.p4_anorm = self.p4_epeak = (
	...
	def fitplot(
		...
		if area_std.mean() > 0:
			fitbox_str += (
				f'Area: <b>{area_str}</b><br>' f'AreaSTD: <b>{areastd_str}</b><br>' f'RMSD: <b>{rmsd:.0E}</b>' f'</span>'
			)
		else:
			fitbox_str += f'Area: <b>{area_str}</b><br>' f'RMSD: <b>{rmsd:.0E}</b>' f'</span>'
	...
	def pcaplot(self, idx: int, attribute_type: str, pca_data: tuple):
		...
		if idx == 0:
			...
			self.g.plot(
				ncomps, expvar, symbol='o', symbolBrush=pretty_colors(1), pen=mkPen(pretty_colors(1), style=Qt.DashLine)
			)
		elif idx in (1, 2, 3):
			pass
		else:
	...
	def plsplot(self, pls_data: dict, mode: str = 'CV'):
		...
		if mode == 'CV':
			...
			plsbox = TextItem(html=plsbox_str, anchor=(0, 1), angle=0, border='#004de6', fill='#ccddff77')
	...
	def guimsg(self, top: str, main: str, tp: str):
		...
		if tp.lower() in ('w', 'warning'):
			...
			df = (
				DataFrame(columns=['Ref_1', '...', 'Ref_n'], index=['Sample_1', '...', 'Sample_n'], data=rand(3, 3) * 10)
				.round(3)
				.to_html()
			)
	...
	def dynamicbox(self, top: str, msg: str, maxi: int):
		...
		:return: None
		"""
		self.mbox = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Information, top, msg, QtWidgets.QMessageBox.NoButton)
	...
	def update_table_from_spreadsheet(self, mode: str, filepath: Path):
		...
		"""
		tables = {
			'Peaks': (self.p3_isotb, ['Element', 'Lower WL', 'Upper WL', 'Center WL', '#Peaks']),
...
class LIBSsaAbout(QtWidgets.QWidget):
	...
	def __init__(self, parent, html: str, version: str):
		...
		self.s1 = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
		self.s2 = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
		# Add layout elements
