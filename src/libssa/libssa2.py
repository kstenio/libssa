...
try:
	...
	from PySide6.QtCore import Qt, QObject, QThreadPool, QCoreApplication
	...
	from libssa.env.spectra import Worker, Spectra
...
class LIBSSA2(QMainWindow):
	...
	def __init__(self, ui_file: str, logo_file: str):
		...
		try:
			...
			QMessageBox.critical(
				QMainWindow(None),
				'Critical error!',
				f'Can not initialize LIBSsa!' f'<p>Error message: <b style="color: red">{str(ex)}</b></p>',
			)
			sys.exit(1)
		else:
	...
	def create_about(self):
		...
		try:
			...
			self.gui.about_html = html.replace('width="500em"', 'width=500 height=70').replace(
			...
			if question == QMessageBox.Yes:
				...
				if save_file.name in ('', '.'):
					...
					if save_file.is_file():
						...
						self.gui.guimsg('Error!', f'Could not save <b>environment</b> data properly. Try free more RAM.', 'c')
						changestatus(self.gui.sb, 'Error when saving environment', 'r', 0)
		elif mode == 'load':
			changestatus(self.gui.sb, 'Please wait, loading environment...', 'b', 1)
			load_file = Path(
				self.gui.guifd(self.parent, 'gof', f'Choose filename to load environment', 'LIBSsa 2.0 Environment (*.lb2e)')[0]
			)
	...
	def loadref(self):
		if not self.spec.samples['Count']:
			...
			ref_file = Path(
				self.gui.guifd(
					self.parent, 'gof', 'Select reference spreadsheet file', 'Excel Spreadsheet Files (*.xls *.xlsx)'
				)[0]
			)
			if str(ref_file) == '.':
				...
				if ref_spreadsheet.index.size != self.spec.samples['Count']:
					self.gui.guimsg(
						...
						'Number of rows <b>must</b> be the same as total of samples!'.format(
							rows=ref_spreadsheet.index.__len__(), samples=self.spec.samples['Count']
						),
						'c',
					)
				else:
	...
	def spreadsheet_to_table(self, mode: str):
		...
		filepath = Path(
			self.gui.guifd(
				self.parent, 'gof', f'Select spreadsheet to update {mode} table', 'Excel 2007+ Spreadsheet (*.xlsx)'
			)[0]
		)
		if filepath.name in ('', '.'):
			self.gui.guimsg('Error', 'Cancelled by the user.', 'w')
	...
	def export_mechanism(self, mode: int):
		...
		if mode == 1:
			...
			fd_params = (Path.home(), 'getExistingDirectory', f'Choose folder for exporting {modes_dict[mode]} data', '')
			index = False
		elif mode == 2:
			suffix = ''
			func = export.export_raw
		...
		try:
			...
			self.gui.guimsg('Error', f'Can not export data for <b>{modes_dict[mode]}</b>.' f'<br>Not implemented yet.', 'c')
			self.gui.sb.clearMessage()
		else:
			path = Path(path)
			if path.name in ('', '.'):
	...
	def doplot(self):
		...
		if self.gui.g_current == 'Raw':
			...
			self.gui.splot(
				self.spec.wavelength['Raw'], self.spec.pearson['Full-Mean'], clear=False, name='Sampling Average Spectrum'
			)
			...
			if idx == 0:
				...
				if idx == 1:
					...
					self.gui.splot(
						self.spec.pca['Transformed'][:, 0], self.spec.pca['Transformed'][:, 1], symbol='o', name='PC2(PC1)'
					)
				if idx == 2:
					self.gui.g.setTitle('Principal component <b>3</b> as function of component <b>1</b>')
					self.gui.splot(
						self.spec.pca['Transformed'][:, 0], self.spec.pca['Transformed'][:, 2], symbol='o', name='PC3(PC1)'
					)
				if idx == 3:
					self.gui.g.setTitle('Principal component <b>3</b> as function of component <b>2</b>')
					self.gui.splot(
						self.spec.pca['Transformed'][:, 1], self.spec.pca['Transformed'][:, 2], symbol='o', name='PC2(PC3)'
					)
				if idx == 4:
	...
	def spload(self):
		...
		if not error:
			...
			self.gui.dynamicbox(
				'Loading data', '<b>Please wait</b>. Loading spectra into LIBSsa...', self.spec.samples['Count']
			)
			worker = Worker(
				load,
			...
			worker.signals.finished.connect(
				lambda: self.gui.updatedynamicbox(val=0, update=False, msg='Spectra loaded into LIBSsa')
			)
	...
	def outliers(self):
		...
		def errors(runerror):
			...
			self.gui.p2_apply_out.setEnabled(True)
			# Outputs timer (error)
			print('Timestamp:', time(), 'ERROR: Could not remove outliers. Timer: %.2f seconds. ' % (time() - self.timer))
			# Outputs error message
			runerror_message = (
		...
		if not self.spec.intensities['Count']:
			...
			worker.signals.progress.connect(self.gui.updatedynamicbox)
			worker.signals.finished.connect(
				lambda: self.gui.updatedynamicbox(val=0, update=False, msg='Outliers removed from set')
			)
			worker.signals.result.connect(result)
	...
	def docorrel(self):
		...
		def result(returned):
			...
			self.gui.p2_apply_correl.setEnabled(True)
			# Outputs timer
			print('Timestamp:', time(), 'MSG: Correlation spectrum count timer: %.2f seconds. ' % (time() - self.timer))
			# Updates gui elements
		...
		self.gui.dynamicbox(
			'Creating correlation spectrum', '<b>Please wait</b>. This may take a while...', self.spec.ref.columns.__len__()
		)
		...
		worker.signals.progress.connect(self.gui.updatedynamicbox)
		worker.signals.finished.connect(
			lambda: self.gui.updatedynamicbox(val=0, update=False, msg='Correlation spectrum for all parameters finished')
		)
		worker.signals.result.connect(result)
		self.configthread()
		self.timer = time()
	...
	def peakiso(self):
		...
		if self.gui.p3_isotb.rowCount() > 0:
			...
			if self.gui.checktablevalues(self.spec.wavelength['Raw'][0], self.spec.wavelength['Raw'][-1]):
				...
				worker.signals.progress.connect(self.gui.updatedynamicbox)
				worker.signals.finished.connect(
					lambda: self.gui.updatedynamicbox(val=0, update=False, msg='Peak isolation finished')
				)
				worker.signals.result.connect(result)
				self.configthread()
				self.timer = time()
	...
	def peakfit(self):
		...
		if not self.spec.isolated['Count']:
			...
			shapes = [
				x.split(')')[1][1:] for x in [self.gui.p3_fittb.cellWidget(y, 1).currentText() for y in range(fittable_rows)]
			]
	...
	def docalibrationcurve(self):
		if (not self.spec.isolated['Count'] and (self.spec.fit['Area'] is self.spec.base)) or (
			self.spec.ref.columns[0] == 'Empty'
		):
			self.gui.guimsg(
				'Warning',
	...
	def pca_perform_scan(self):
		...
		if mode == 'Raw':
			...
			if not self.spec.samples['Count']:
				self.gui.guimsg(
					'Error', 'You have to import samples <b style="color: red">before</b> performing components scan!', 'w'
				)
				ok = False
			else:
			...
			if not self.spec.isolated['Count']:
				self.gui.guimsg(
					'Error', 'Please perform peak isolation <b style="color: red">before</b> using this feature.', 'w'
				)
				ok = False
			else:
			...
			if self.spec.fit['Shape'] is self.spec.base:
				self.gui.guimsg(
					'Error', 'Please perform peak fitting <b style="color: red">before</b> using this feature.', 'w'
				)
				ok = False
			else:
	...
	def pls_predict(self):
		if self.spec.pca['Transformed'] is self.spec.base:
			self.gui.guimsg(
				'Warning', 'You must <i>create attributes from PCA</i> <b style="color:red">before</b> using this feature.', 'w'
			)
		else:
			# This means all attributes are fine.
	...
	def calc_t_ne(self):
		try:
			...
			result = tne_do(
				self.spec.samples['Name'], array(self.spec.fit[parameter]).T.squeeze(), df_element, self.gui.p6_ion.text()
			)
		except Exception as ex:
			self.gui.guimsg(
				'Error', f'Could not calculate T/Ne.<p>' f'Error message: <b style="color: red">{str(ex)}</b>.</p>', 'c'
			)
			print_exc()
		else:
			# Saves result
