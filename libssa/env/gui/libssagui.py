...
class LIBSsaGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, uifile: str, logofile: str):
        ...
        try:
            ...
            self.peaknormitems = []
            self.version = version('libssa')
            self.about_html = ''
        except Exception as err:
            raise ValueError(
                'Could not initialize UI file. Error message:\n\t%s' % str(err)
            )
        # If no error was found, loads all remaining widgets
            ...
            # Menubar
            self.menu_file_load = self.menu_file_save = self.menu_file_sample = (
                self.menu_file_quit
            ) = QtGui.QAction()
            self.menu_import_ref = self.menu_import_peaks = self.menu_import_tne = (
                QtGui.QAction()
            )
            self.menu_export_fullspectra_raw = self.menu_export_fullspectra_out = (
                QtGui.QAction()
            )
            self.menu_export_peaks_table = self.menu_export_peaks_isolated = (
                self.menu_export_peaks_fitted
            ) = self.menu_export_peaks_areas = QtGui.QAction()
            self.menu_export_predictions_linear = self.menu_export_predictions_pls = (
                QtGui.QAction()
            )
            self.menu_export_other_pca = self.menu_export_other_tne = (
                self.menu_export_other_correl
            ) = QtGui.QAction()
            ...
            self.p1_header = self.p1_wcol = self.p1_ccol = self.p1_dec = (
                QtWidgets.QSpinBox()
            )
            ...
            self.p3_isoadd = self.p3_isorem = self.p3_isoapply = self.p3_fitapply = (
            ...
            self.p4_areas = self.p4_heights = self.p4_wnorm = self.p4_pnorm = (
                self.p4_anorm
            ) = self.p4_epeak = QtWidgets.QRadioButton()
            self.p4_apply = QtWidgets.QPushButton()
            self.p4_npeak = self.p4_npeak_norm = QtWidgets.QSpinBox()
            # Page 5 == PCA and PLSR
            self.p5_pca_raw = self.p5_pca_iso = self.p5_pca_areas = (
                self.p5_pca_heights
            ) = QtWidgets.QRadioButton()
            self.p5_pca_fs = QtWidgets.QCheckBox()
            self.p5_pca_ncomps = QtWidgets.QSpinBox()
            self.p5_pca_cscan = self.p5_pca_do = self.p5_pls_cal_start = (
                self.p5_pls_pred_start
            ) = QtWidgets.QPushButton()
            self.p5_pls_cal_att = self.p5_pls_pred_model = self.p5_pls_pred_att = (
                QtWidgets.QLabel()
            )
    ...
    def loadmain(self):
        ...
        self.menu_export_predictions_linear = self.mw.findChild(
            QtGui.QAction, 'actionE07'
        )
    ...
    def connects(self):
        ...
        pass
        # P6
        self.p6_element.currentIndexChanged.connect(
            lambda: self.update_tne_values(False)
        )
        ...
        self.p3_isotb.horizontalHeader().setSectionResizeMode(
            QtWidgets.QHeaderView.Stretch
        )
        self.p3_fittb.horizontalHeader().setSectionResizeMode(
            QtWidgets.QHeaderView.Stretch
        )
    ...
    def setgoptions(self):
        ...
        # PCA plot
            ...
            if pca == 1:
                ...
                self.g_op = [
                    'Principal Component <b>1</b>',
                    'a.u.',
                    'Principal Component <b>2</b>',
                    'a.u.',
                ]
            elif pca == 3:
                self.g_op = [
    ...
    def splot(
        ...
        if symbol == '':
            self.g.plot(
                x.reshape(-1),
                y.reshape(-1),
                pen=mkPen(randint(50, 200, (1, 3))[0], width=width),
                name=name,
            )
        else:
            self.g.plot(
                ...
                pen=None,
                symbol=symbol,
                symbolBrush=mkBrush(randint(50, 200, (1, 3))[0]),
                name=name,
            )
        self.g.autoRange()
    ...
    def fitplot(
        ...
        rmsd = std(data[:, 1])
        x = (
            linspace(wavelength_iso[0], wavelength_iso[-1], 1000)
        ...
        self.g.plot(
            ...
            symbol='o',
            pen=None,
            symbolBrush=mkBrush(randint(50, 220, (1, 3))[0]),
            name='Original data',
        )
        self.g.plot(
            wavelength_iso,
            ...
            symbol='+',
            pen=None,
            symbolBrush=mkBrush(list(randint(50, 220, (1, 3))[0]) + [0.8]),
            name='Residuals',
        )
        ...
        for i in range(len(area)):
            self.g.plot(
                x,
                total[:, i],
                pen=mkPen(randint(50, 220, (1, 3))[0], width=1),
                name='Peak %i' % (i + 1),
            )
        ...
        self.g.plot(
            x,
            total[:, -1],
            pen=mkPen(randint(50, 220, (1, 3))[0], width=2.5),
            name='Total',
        )
        # Organizes string for fit box
        fitbox_str = (
            f'<span style="color:#330066">'
        ...
        if area_std.mean() > 0:
            ...
            fitbox_str += (
                f'Area: <b>{area_str}</b><br>' f'RMSD: <b>{rmsd:.0E}</b>' f'</span>'
            )
        fitbox = TextItem(
            html=fitbox_str, anchor=(1, 0), angle=0, border='#6600cc', fill='#f2e6ff'
        )
    ...
    def linplot(self, linear: dict, index: int):
        ...
        linbox = TextItem(
            html=linbox_str, anchor=(0, 1), angle=0, border='#004de6', fill='#ccddff77'
        )
    ...
    def pcaplot(self, idx: int, attribute_type: str, pca_data: tuple):
        ...
        if idx == 0:
            ...
            self.g.plot(
                ...
                expvar,
                symbol='o',
                symbolBrush=pretty_colors(1),
                pen=mkPen(pretty_colors(1), style=Qt.DashLine),
            )
        elif idx in (1, 2, 3):
            pass
        else:
            ...
            self.g.plot(
                x[sort],
                ld[sort][:, 0],
                pen=mkPen(pretty_colors(1), width=2),
                name='Loadings PC1',
            )
            self.g.plot(
                x[sort],
                ld[sort][:, 1],
                pen=mkPen(pretty_colors(1), width=1.5),
                name='Loadings PC2',
            )
            self.g.plot(
                x[sort],
                ld[sort][:, 2],
                pen=mkPen(pretty_colors(1), width=1),
                name='Loadings PC3',
            )
            self.g.plot(
                x[sort],
                zeros(x.size),
                pen=mkPen('#aaaaaa', width=0.8, style=Qt.DashLine),
            )
        self.g.autoRange()
    ...
    def plsplot(self, pls_data: dict, mode: str = 'CV'):
        ...
        if mode == 'CV':
            ...
            self.g.addItem(
                BarGraphItem(x=x_pred, height=prediction, width=0.9, brushes=colors)
            )
        else:
    ...
    def saha_b_plot(self, plasma_params: dict, idx: int):
        ...
        t, st, ne, sne, r2, r = plasma_params['Report'].loc[
            plasma_params['Report'].index[idx]
        ]
        pbox_str = (
            f'<span style="color:#330066">'
            f'T: <b>{t:.0f} K</b><br>'
        ...
        pbox = TextItem(
            html=pbox_str, anchor=(0, 1), angle=0, border='#004de6', fill='#ccddff'
        )
        self.g.addItem(pbox)
    ...
    def guimsg(self, top: str, main: str, tp: str):
        ...
        if tp.lower() in ('w', 'warning'):
            ...
            return QtWidgets.QMessageBox.information(
                self.mw, top, f'{main}<p style="text-align: center">{df}</p>'
            )
        else:
    ...
    def guifd(self, parent: Path, tp: str, caption: str, fd_filter: str = ''):
        ...
        if tp in ('ged', 'getExistingDirectory'):
            return QtWidgets.QFileDialog.getExistingDirectory(
                parent=self.mw, caption=caption, dir=str(parent)
            )
        elif tp in ('gof', 'getOpenFileName'):
            return QtWidgets.QFileDialog.getOpenFileName(
                parent=self.mw, caption=caption, filter=fd_filter, dir=str(parent)
            )
    ...
    def dynamicbox(self, top: str, msg: str, maxi: int):
        ...
        """
        self.mbox = QtWidgets.QMessageBox(
            QtWidgets.QMessageBox.Information, top, msg, QtWidgets.QMessageBox.NoButton
        )
        ...
        mbox_layout.addWidget(
            self.mbox_pbar,
            mbox_layout.rowCount(),
            0,
            1,
            mbox_layout.columnCount(),
            Qt.AlignCenter,
        )
        self.mbox.show()

    def updatedynamicbox(
        self, val: int, update: bool = True, msg: str = 'Operation Finished'
    ):
        """
    ...
    def config_fsn(self, mode: int):
        ...
        if mode == 0:
            ...
            en_dis = (
                True if self.p1_fsn_type.currentText() == 'Internal Standard' else False
            )
            ...
            if self.p1_fsn_lminus.value() >= self.p1_fsn_lplus.value():
                self.guimsg(
                    'Invalid value',
                    'Lower wavelenght must be <b style="color: red">lower</b> than upper!',
                    'w',
                )
            ...
            if self.p1_fsn_lplus.value() <= self.p1_fsn_lminus.value():
                self.guimsg(
                    'Invalid value',
                    'Upper wavelenght must be <b style="color: red">bigger</b> than lower!',
                    'w',
                )
    ...
    def changetable(self, option: bool):
        ...
        if selected.__len__() > 0:
            ...
            if option:
                ...
                self.p3_isotb.setItem(
                    self.p3_isotb.rowCount() - 1, 4, QtWidgets.QTableWidgetItem('1')
                )
            else:
                self.p3_isotb.setRowCount(self.p3_isotb.rowCount() - 1)
    ...
    def checktable(self, row: int, col: int):
        ...
        if not len(value) or '---' in value:
            error[0], error[1] = (
                True,
                'Cell can not be <b>empty</b>! Please assign a value.',
            )
        # Now, we know cell is not empty, but must check if value are OK
            ...
            if col == 0:
                ...
                element_value = (
                    self.p3_isotb.item(row, col).text().title().replace(' ', '_')
                )
                ...
                if element_value in element_values:
                    error[0] = True
                    error[1] = (
                        'The element <b>%s</b> is already in the table.' % element_value
                    )
                else:
            ...
            # Lower and upper wavelengths columns
                ...
                try:
                    ...
                    error[0], error[1] = (
                        True,
                        'This cell can only accept <b>numbers</b>.',
                    )
            # Center column
            elif col == 3:
                for char in value:
                    if (char not in ('.', ';')) and (char in punctuation):
                        error[0] = True
                        error[1] = (
                            'This cell can only accept <b>numbers</b> <i>or</i> <b>numbers separated by semicolon</b>.'
                        )
                        break
                else:
            ...
            # Peak number column
                ...
                try:
                    ...
                    error[0], error[1] = (
                        True,
                        'This cell can only accept <b>integers</b>.',
                    )
    ...
    def checktablevalues(self, w_lower: float, w_upper: float):
        ...
        for r in range(rows):
            try:
                ...
                self.guimsg(
                    'Critical error', 'Empty value in <b>element</b> name!', 'c'
                )
                return False
            else:
                try:
                    [lower, upper] = list(
                        map(
                            float,
                            [
                                self.p3_isotb.item(r, 1).text(),
                                self.p3_isotb.item(r, 2).text(),
                            ],
                        )
                    )
                except AttributeError:
                    self.guimsg(
                        'Critical error',
                        '<b>Upper</b> or <b>lower</b> cells have empty values!',
                        'c',
                    )
                    return False
                else:
                    ...
                    if lower < w_lower or upper > w_upper:
                        self.guimsg(
                            'Critical error',
                            '<b>Upper</b> or <b>lower</b> wavelengths are not inside spectra range!'
                            f'<p>Element: <b>{element}</b>'
                            f'<br>Spectra range: <b>{w_lower}</b> to <b>{w_upper}</b>'
                            f'<br><i>Table entered range</i>: <b>{lower}</b> to <b>{upper}</b></p>',
                            'c',
                        )
                        return False
                    if lower < upper:
                        ...
                        try:
                            ...
                            self.guimsg(
                                'Critical error',
                                '<b>Center</b> or <b>#Peaks</b> cells have empty values!',
                                'c',
                            )
                            return False
                        else:
                            if len(center) == peaks:
                                ...
                                for c in center:
                                    if lower < float(c) < upper:
                                        ...
                                        error_str = (
                                            'Peak center <b>must</b> be between lower and upper values!<p>'
                                            'Element: <b>{e}</b><br><br>'
                                            'Lower: <b>{l}</b><br>'
                                            'Center: <b>{c}</b><br>'
                                            'Upper: <b>{u}</b>'
                                            '</p>'.format(
                                                e=element, u=upper, l=lower, c=float(c)
                                            )
                                        )
                                ...
                                error_str = (
                                    'Number of peaks does not have the same size as center list!<p>'
                                    'Element: <b>{e}</b><br><br>'
                                    '#Peaks: <b>{p}</b><br>'
                                    'Center: <b>{c}</b>'
                        ...
                        error_str = (
                            'Invalid values for upper and lower wavelengths!<p>'
                            'Element: <b>{e}</b><br><br>'
                            'Lower: <b>{l}</b><br>'
                            'Upper: <b>{u}</b>'
    ...
    def create_fit_table(self):
        ...
        for r in range(rows):
            ...
            self.p3_fittb.setItem(
                r, 0, QtWidgets.QTableWidgetItem(self.p3_isotb.item(r, 0).text())
            )
    ...
    def setpeaknorm(self):
        ...
        self.peaknormitems = [
            self.p4_peak.itemText(i)
            for i in range(self.p4_peak.count())
            if self.p4_peak.itemText(i) != self.p4_peak.currentText()
        ]
    ...
    def update_tne_values(self, start: bool = False):
        ...
        if start:
            ...
            self.p6_table.horizontalHeader().setSectionResizeMode(
                QtWidgets.QHeaderView.Stretch
            )
            # Creates DF-Structure for each element
            self.p6_table_dfs = {
                e: DataFrame(columns=['Element', 'Ionization', 'gAk', 'Ek'], dtype=str)
                for e in elements
            }
        ...
        if self.p6_table_dfs[element].index.size <= 2:
            ...
            for fr in range(fit_rows):
                ...
                if element in fit_element:
                    ...
                    self.p6_table_dfs[element].loc[fit_element] = list(
                        map(str, (fit_element, ionization, 0, 0))
                    )
        ...
        for r in range(rows):
            for c in range(cols):
                self.p6_table.setItem(
                    r, c, QtWidgets.QTableWidgetItem(df_element.iloc[r, c])
                )
    ...
    def check_tne_table(self, row: int, col: int):
        ...
        try:
            ...
            self.guimsg(
                ...
                f'Restoring the last saved value.</p>',
                'c',
            )
            restore = True
        else:
    ...
    def update_table_from_spreadsheet(self, mode: str, filepath: Path):
        ...
        tables = {
            'Peaks': (
                self.p3_isotb,
                ['Element', 'Lower WL', 'Upper WL', 'Center WL', '#Peaks'],
            ),
        ...
        if df.columns.tolist() != tables[mode][1]:
            self.guimsg(
                'Error',
                f'Wrong columns names for <b>{mode} table</b>!<p>'
                f'<b style="color: blue">Allowed</b>: <u>{", ".join(tables[mode][1])}</u><br>'
                f'<b style="color: red">Entered</b>: <u>{", ".join(df.columns.tolist())}</u></p>',
                'c',
            )
        else:
...
def changestatus(
    bar: QtWidgets.QStatusBar, message: str = '', color: str = '', bold: bool = False
):
    """
    ...
    if (message == '') and (color == ''):
        ...
        if len(color) == 1:
            ...
            if color == 'r':
                ...
                raise ValueError(
                    'Wrong color identifier. Use "r","g","b","p" or 6 hexadecimal values.'
                )
        if len(color) == 6:
            ...
            raise ValueError(
                'Wrong color length style. Please use 6 hexadecimal values.'
            )
...
class LIBSsaAbout(QtWidgets.QWidget):
    ...
    def __init__(self, parent, html: str, version: str):
        ...
        self.label = QtWidgets.QLabel(
            f'<b>About LIBSsa version <u style="color: blue">{version}</u></b>'
        )
        self.text = QtWidgets.QTextEdit(html)
        self.ok = QtWidgets.QPushButton('OK')
        self.vlayout = QtWidgets.QVBoxLayout()
        self.hlayout = QtWidgets.QHBoxLayout()
        self.s1 = QtWidgets.QSpacerItem(
            40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        self.s2 = QtWidgets.QSpacerItem(
            40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        # Add layout elements
